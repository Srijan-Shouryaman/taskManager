          ...
          <div style={{ marginLeft: '10px' }}>
            {logs.map((log, index) => (
              <div key={log.id} style={{ borderLeft: '2px solid var(--accent)', paddingLeft: '24px', position: 'relative', paddingBottom: index === logs.length - 1 ? '0' : '32px' }}>
                <div style={{ position: 'absolute', left: '-7px', top: '0', width: '12px', height: '12px', borderRadius: '50%', background: 'var(--accent)', border: '2px solid var(--bg-main)' }}></div>
                <p style={{ fontSize: '16px', color: 'var(--text-main)', marginBottom: '8px', fontWeight: '500', lineHeight: '1.4' }}>{log.text}</p>
